...
export class ContentRenderer {
    private static readonly DEFAULT_OPTIONS: RenderOptions = {
    ...
    private static readonly SYNTAX_HIGHLIGHTING_CSS = `
        <style>
     ...
     */
    private static parseSections(content: string): ContentSection[] {
        ...
        for (const line of lines) {
            ...
            if (headingMatch) {
                ...
                currentSection = {
                    ...
                    title,
                    content: '',
                    collapsible: level >= 2, // Make h2 and below collapsible
                    collapsed: false
                };
     ...
     */
    private static generateTableOfContents(content: string): TableOfContentsEntry[] {
     ...
     */
    private static renderMarkdownToHtml(content: string, options: RenderOptions): string {
        let html = content;
     ...
     * Apply syntax highlighting to code blocks
     */
    private static applySyntaxHighlighting(html: string, codeBlocks: CodeBlock[]): string {
     ...
     */
    private static addCollapsibleSections(html: string, sections: ContentSection[]): string {
     ...
     */
    private static addTableOfContents(html: string, toc: TableOfContentsEntry[]): string {
        const tocHtml = `
     ...
     */
    private static getInteractionScript(options: RenderOptions): string {
        return `
            <script>
 ...
 */
export function renderMarkdown(content: string, options?: Partial<RenderOptions>): string {
