...
class SettingsPanel {
    static createOrShow(extensionUri, context) {
        ...
        const panel = vscode.window.createWebviewPanel(SettingsPanel.viewType, '⚙️ Spec-Bot Settings', column || vscode.ViewColumn.One, {
    ...
    async _resetSettings() {
        try {
            ...
            const resetApiKeys = await vscode.window.showWarningMessage('Do you also want to clear all saved API keys?', { modal: true }, 'Yes, clear API keys', 'No, keep API keys');
    ...
    async _testApiKey(provider, key) {
        try {
...
SettingsPanel.viewType = 'spec-bot-settings';
